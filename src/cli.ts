...
function resolveBaseDir(dirOption?: string): string {
  ...
  if (process.env.AGENT247_BASE_DIR) return resolve(process.env.AGENT247_BASE_DIR);
...
program
  ...
  .action((taskId: string) => runCommand(taskId, resolveBaseDir(program.opts().dir)));
...
program
  ...
  .action((options) => statusCommand(resolveBaseDir(program.opts().dir), options));

program
  .command("show <ulid>")
  .description("Display a run report")
  .action((ulid: string) => showCommand(ulid, resolveBaseDir(program.opts().dir)));

program
  .command("review <ulid>")
  .description("Mark a run as reviewed")
  .action((ulid: string) => reviewCommand(ulid, resolveBaseDir(program.opts().dir)));
...
program
  ...
  .action((options) => cleanCommand(resolveBaseDir(program.opts().dir), options));
